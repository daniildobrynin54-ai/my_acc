"""Модуль для отслеживания дневной статистики с учетом часового пояса MSK и вкладов сверх лимита."""
...
from config import (
    ...
    TIMEZONE_OFFSET,
    EXTRA_DONATIONS_LIMIT
)

class DailyStatsManager:
    """Менеджер дневной статистики с учетом MSK, сброса лимитов и вкладов сверх лимита."""
    ...
    def fetch_stats_from_page(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 🔧 НОВОЕ: Вычисляем лимиты с учётом EXTRA_DONATIONS_LIMIT
            base_max = donations_max
            effective_max = base_max + EXTRA_DONATIONS_LIMIT
            donations_left = effective_max - donations_used
            
            # Флаг превышения базового лимита
            over_base_limit = donations_used >= base_max
            extra_used = max(0, donations_used - base_max) if over_base_limit else 0
            extra_left = max(0, EXTRA_DONATIONS_LIMIT - extra_used)
            
            stats = {
                "donations_used": donations_used,
                "donations_max": donations_max,  # Базовый лимит со страницы
                "donations_left": donations_left,
                ...
                "extra_donations_enabled": EXTRA_DONATIONS_LIMIT > 0,
                "extra_donations_limit": EXTRA_DONATIONS_LIMIT,
                "effective_max": effective_max,
                "over_base_limit": over_base_limit,
    ...
    def get_stats(self, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        if force_refresh or self._cached_stats is None:
            ...
            if stats is None:
                ...
                base_max = MAX_DAILY_DONATIONS
                effective_max = base_max + EXTRA_DONATIONS_LIMIT
                
                return {
                    "donations_used": 0,
                    "donations_max": base_max,
                    "replacements_used": 0,
                    "replacements_max": MAX_DAILY_REPLACEMENTS,
                    ...
                    "extra_donations_limit": EXTRA_DONATIONS_LIMIT,
                    "effective_max": effective_max,
                    "over_base_limit": False,
                    "extra_used": 0,
                    "extra_left": EXTRA_DONATIONS_LIMIT
                }
    ...
    def can_donate(self, force_refresh: bool = True) -> bool:
        """
        🔧 ОБНОВЛЕНО: Проверяет, можно ли пожертвовать карту с учётом EXTRA_DONATIONS_LIMIT.
        
        Args:
            force_refresh: Обновить данные с сервера
        
        Returns:
            True если лимит не достигнут (включая дополнительный)
        """
    ...
    def print_stats(self, force_refresh: bool = False) -> None:
        """🔧 ОБНОВЛЕНО: Выводит текущую статистику с информацией о дополнительных вкладах."""
        stats = self.get_stats(force_refresh=force_refresh)
        ...
        # Отображение пожертвований
        if stats["extra_donations_enabled"]:
            base_max = stats["donations_max"]
            effective_max = stats["effective_max"]
            used = stats["donations_used"]
            
            if stats["over_base_limit"]:
                # Превышен базовый лимит
                extra_used = stats["extra_used"]
                extra_left = stats["extra_left"]
                print(f"   Пожертвовано: {used}/{effective_max} ({base_max} + {EXTRA_DONATIONS_LIMIT} доп.)")
                print(f"   ⚠️  БАЗОВЫЙ ЛИМИТ ПРЕВЫШЕН! Использовано доп: {extra_used}/{EXTRA_DONATIONS_LIMIT}")
                print(f"   Осталось доп. вкладов: {extra_left}")
            else:
                # В пределах базового лимита
                print(f"   Пожертвовано: {used}/{effective_max} ({base_max} + {EXTRA_DONATIONS_LIMIT} доп.)")
                print(f"   Осталось до базового лимита: {base_max - used}")
                print(f"   Осталось всего: {stats['donations_left']}")
        else:
